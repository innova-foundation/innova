...
bool CalculateExpiresAt(CNameRecord& nameRec)
{
    ...
    for(unsigned int i = nameRec.nLastActiveChainIndex; i < nameRec.vtxPos.size(); i++)
    {
        ...
        if (!DecodeNameTx(tx, nti))
            ...
            sum += nti.nRentalDays * 5760; //days to blocks. 5760 is average number of blocks per day roughly (based on 15 second blocks)
    }
...
// scans innovanames.dat and return names with their last CNameIndex
...
bool CNameDB::ReadName(const CNameVal& name, CNameRecord& rec)
{
     ...
     if (s > 0 && rec.nLastActiveChainIndex >= s) //&& rec.nLastActiveChainIndex >= s
    {
        ...
        LogPrintf("innovanames.dat is corrupt! It will be recreated on next start.");
...
bool IsNameFeeEnough(const CTransaction& tx, const NameTxInfo& nti, const CBlockIndex* pindexBlock, const CAmount& txFee)
{
    ...
    for (int i = 1; i <= 10000; i++) //Original 10, now 10,000 PoW Blocks
    {
...
Value name_new(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
                ...
                "Cost is 0.9 INN To TX Fees (Miners/Stakers) and 0.1 INN To Name Registration."
...
Value name_delete(const Array& params, bool fHelp)
{
    ...
    if (!IsSynchronized())
        throw runtime_error("Innova is still downloading - wait until it is done.");
...
NameTxReturn name_operation(const int op, const CNameVal& name, CNameVal value, const int nRentalDays, const string strAddress, bool fValueAsFilepath)
{
    ...
    if (IsInitialBlockDownload())
    {
        ...
        ret.err_msg = "Innova is downloading blocks...";
        return ret;
    }
    ...
    // add destination to namescript
        if ((op == OP_NAME_UPDATE || op == OP_NAME_NEW) && strAddress != "")
        {
            ...
            if (!address.IsValid())
            {
                ...
                ret.err_msg = "Innova address is invalid";
                return ret;
            }
...
// {
//     printf("Scanning Innova's chain for names to create a fast index...\n");

//     // create empty innovanames.dat
//     CNameDB dbName("cr+");

//     // scan blockchain to fill innovanames.dat
//     CTxDB txdb("r");
...
{
    LogPrintf("Scanning Innova blockchain for names to create a fast index...\n");
    //CNameDB dbName("cr+");
...
bool CNamecoinHooks::CheckInputs(const CTransaction& tx, const CBlockIndex* pindexBlock, vector<nameTempProxy> &vName, const CDiskTxPos& pos, const CAmount& txFee)
{
    ...
    if (!nameRec.vtxPos.empty() && !nameRec.deleted())
    {
        ...
        if (!DecodeNameTx(lastKnownNameTx, prev_nti))
            return error("CheckInputsHook() : Failed to decode existing previous name tx for %s. Your blockchain or innovanames.dat may be corrupt.", info.c_str());
    ...
    // all checks passed - record tx information to vName. It will be sorted by nTime and writen to innovanames.dat at the end of ConnectBlock
    CNameIndex txPos2;
...
bool CNamecoinHooks::DisconnectInputs(const CTransaction& tx)
{
...

    {
        ...
        {
            // check if tx matches last tx in innovanames.dat
            CTransaction lastTx;
...
// Executes name operations in vName and writes result to innovanames.dat.
...
bool CNamecoinHooks::ConnectBlock(CBlockIndex* pindex, const vector<nameTempProxy> &vName)
{
    ...
    // All of these name ops should succed. If there is an error - innovanames.dat is probably corrupt.
    CNameDB dbName("r+");
    ...
    BOOST_FOREACH(const nameTempProxy& i, vName)
    {
        ...
        LogPrintf("ConnectBlockHook(): writing %s %s in block %d to innovanames.dat\n", stringFromOp(i.op), stringFromNameVal(i.name), pindex->nHeight);
