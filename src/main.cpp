...
if (fTestNet) {
  if (pindexBest->nHeight == 1)
      nSubsidy = 3000000 * COIN;  // 3m Innova Premine for testing
  ...
  if (pindexBest->nHeight == 1)
    ...
    nSubsidy = 0.825 * COIN;
  	...
  	nSubsidy = 0.0825 * COIN;
        ...
        //else if (pindexBest->nHeight > 1500000) // Start PoW Rewards again as 0.0001 INN per block, ~200 INN per year - Block 1.5 million
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck, bool fWriteNames)
{
    ...
    */
    ...
    // Then, the word COLLATERALNODES came to the table.
    ...
    // ----- Innova collateral nodes, the fair payment edition  -----
    ...
    if (fTestNet) {
        if (pindex->nHeight > BLOCK_START_COLLATERALNODE_PAYMENTS_TESTNET){ // Block 550 Testnet
        ...
        if (pindex->nHeight > BLOCK_START_COLLATERALNODE_PAYMENTS){ //Block 800 Mainnet
            CollateralnodePayments = true;
            if(fDebug) { printf("CheckBlock() : Collateralnode payments enabled\n"); }
        }else{
            CollateralnodePayments = false;
                                            ...
                                            if (!CheckPoSCNPayment(pindex, vtx[1].vout[i].nValue, mn)) // CheckPoSCNPayment()
                                            {
                                              if (pindexBest->nHeight >= MN_ENFORCEMENT_ACTIVE_HEIGHT || pindexBest->nHeight >= MN_ENFORCEMENT_ACTIVE_HEIGHT_TESTNET) { //Update PoS CN Payments to not go out of sync
                                                  printf("CheckBlock-POS() : Out-of-cycle collateralnode payment detected, rejecting block.");
                                                    ...
                                                    printf("CheckBlock-POS(): This collateralnode payment is too aggressive and will not be accepted after block %d\n", MN_ENFORCEMENT_ACTIVE_HEIGHT);
                                                }
                                              //  break;
                                            } else {
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // innova: add in nTx, nChainWork, and nChainTx
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        ...
        nStakeMinAge = 8 * 60 * 60; // test net min age is 8 hours
    ...
    if (mapBlockIndex.empty())
    {
      ...
      if(fTestNet)
      {
        const char* pszTimestampTestNet = "Innova TestNet";
            CTransaction txNewTestNet;

            txNewTestNet.nTime = 1611721726;
