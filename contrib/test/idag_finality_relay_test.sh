...
log "Starting 3-node local regtest mesh"
for node in 0 1 2; do
    start_node "$node"
    sleep 1
done
...
connect_mesh
for node in 0 1 2; do
    if wait_peer_count "$node" 1; then
...
mine_until_height 0 8 || { fail "pre-DAG mining failed"; exit 1; }
...
mine_until_height 0 11 || { fail "DAG activation mining failed"; exit 1; }
...
wait_all_height "$PAYLOAD_HEIGHT" || { fail "vote/share payload block did not relay"; exit 1; }
...
if ! is_int "$CERT_COUNT" || [ "$CERT_COUNT" -lt 1 ]; then
    ...
    if wait_for_pending_private_cert; then
        ...
        wait_all_height "$CERT_HEIGHT" || { fail "certificate payload block did not relay"; exit 1; }
