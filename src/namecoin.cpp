...
bool DecodeNameScript(const CScript& script, NameTxInfo& ret, CScript::const_iterator& pc, bool checkValuesCorrectness, bool checkAddressAndIfIsMine)
{
    ...
    ret.err_msg = "failed to read delimeter inn in: name << rental << inn << value";
    if (!script.GetOp(pc, opcode))
        return false;
    if (opcode != OP_2DROP)
        return false;
...
Value name_new(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
                ...
                "Cost is 0.9 INN To TX Fees and 0.01 To Name Registration."
                + HelpRequiringPassphrase());
...
//     // create empty innovanameindex.dat
//     CNameDB dbName("cr+");

//     // scan blockchain to fill innovanameindex.dat
//     CTxDB txdb("r");
        ...
        vector<nameTempProxy>& vName)
{
    ...
    // all checks passed - record tx information to vName. It will be sorted by nTime and writen to innovanamesindex.dat at the end of ConnectBlock
    CNameIndex txPos2;
    txPos2.nHeight = pindexBlock->nHeight;
...
// Executes name operations in vName and writes result to innovanamesindex.dat.
...
// if pending is true it will grab value from pending names. If there are no pending names it will grab value from innovanamesindex.dat
